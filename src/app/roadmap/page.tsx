    ...
    <main className="flex flex-col items-center justify-center w-full">
      <div className="flex items-center justify-center min-h-[60vh] bg-gradient-token py-16 w-full">
        <div className="w-[80vw] lg:w-[40vw] m-auto lg:text-center">
          <h2 className={`${archivo_black.className} uppercase text-3xl lg:text-5xl mb-5 text-secondary`}>
            Road<span className="text-accent">map</span>
          </h2>
          <p>
            We have our entire journey mapped out. Know exactly what we are up to reading through our roadmap.
          </p>
        </div>
      </div>
            ...
            return (
              <div key={index}>
                <div className="relative flex flex-col gap-16">
                  <div className="flex">
                    <div className="flex p-3 px-8 bg-white text-primary rounded-full">
                      {yearKey}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-8 lg:gap-0 lg:grid-cols-[auto_1fr_auto_0.3fr] items-start justify-between">
                    {quarters[0] && (
                      <RoadmapCard quarter={quarters[0].quarter} items={quarters[0].items} />
                    )}
                    <div className="hidden lg:flex items-center h-full">
                      <div className="border-t-2 border-dashed border-spacing-[3px] border-accent w-full" />
                    </div>
                    {quarters[1] && (
                      <RoadmapCard quarter={quarters[1].quarter} items={quarters[1].items} />
                    )}
                    {quarters[1] && (
                      <div
                        className={`relative hidden lg:flex items-center h-full ${
                          quarters[1].items.length > 4 ? 'top-[34.6%]' : 'top-[56%]'
                        }`}
                      >
                        <div
                          className={`border-t-2 border-r-2 border-b-2 border-dashed border-spacing-[3px] border-accent ${
                            quarters[1].items.length > 4 ? 'h-[70%]' : 'h-full'
                  ...
                  {quarters[1] && (
                    <div
                      className={`hidden lg:inline-block absolute border-t-2 border-l-2 border-dashed border-spacing-[3px] border-accent h-[300px] ${
                        quarters[1].items.length > 4 ? 'top-[50.5%]' : 'top-[48.7%]'
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-[auto_1fr_auto_0.3fr] gap-8 lg:gap-0 items-start justify-between lg:mt-[200px]">
                    {quarters[2] && (
                      <RoadmapCard quarter={quarters[2].quarter} items={quarters[2].items} />
                    )}
                    <div className="hidden lg:flex items-center h-full">
                      <div className="border-t-2 border-dashed border-spacing-[3px] border-accent w-full" />
                    </div>
                    {quarters[3] && (
                      <RoadmapCard quarter={quarters[3].quarter} items={quarters[3].items} />
                    )}
