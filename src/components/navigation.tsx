    ...
    return (
            <div className={`${(modalIslOpen || gameModalIslOpen) ? 'bg-transparent':'bg-primary-dark' } md:bg-transparent md:h-auto flex items-start justify-center w-[100vw] overflow-x-clip`}>
                ...
                <div className={'flex items-center justify-between w-[90vw] lg:w-[80vw] p-5'}>
                    <Link href={'/'}>
                        <div className={'relative h-[30px] w-[150px] lg:h-[50px] lg:w-[250px]'}>
                    ...
                    <div className={`fixed top-0 left-0 w-[100vw] lg:w-auto h-[100vh] md:h-auto lg:relative bg-primary-dark lg:bg-transparent z-[99] transition-all ${showNavigation ? 'left-0' : 'left-[100vw] lg:left-0'}`}>
                        ...
                        <div className={`flex flex-col md:flex-row items-center justify-center gap-5 h-full`}>
                            {
                                [
                                    ...
                                    {url: '#', text: '', component: (
                                            <div className={'md:flex relative cursor-pointer'} key={Date.now()}>
                                                <div className={`group relative ${poppins.bold.className}`}
                                                    ...
                                                    }} className={`hover:text-accent ${activeLink.startsWith('/game') ? 'text-accent': 'text-white'} rounded-lg text-center`}>Game</button>

                                                    <div
                                                        onMouseLeave={()=> setShowGameShowDropdown(0)}
                                                        onMouseEnter={()=> setShowGameShowDropdown(2)} id={'game-dropdown'}
                                                        className={`hidden md:inline transition-all absolute  overflow-clip ${showGameDropdown > 0 ? 'flex opacity-100 visible top-12':'opacity-0 invisible top-32'} flex-col z-40 w-[250px] h-auto bg-primary rounded-xl`}>
                                                        ...
                                                        <Link href={'/game/map'} className={'hover:bg-primary-dark'}
                                                              onClick={()=>{
                                    ...
                                    {url: 'https://stake.brntoken.net', text: 'Stake', newTab: true},
                                    ...
                                    link.component ? (link.component) :
                                        <Link href={link.url} target={link.newTab ? '_blank': '_self'} key={index} className={`${activeLink == link.url ? 'text-accent': ''} hover:text-accent ${poppins.bold.className}`}>
                                            <button onClick={()=> {
                            ...
                            <button onClick={()=>{
                                toggleNavigation();
                                setModalIsOpen(true);
                            }} className={'md:hidden bg-white text-primary p-2 rounded-lg min-w-[100px] text-center'}>Buy</button>

                            <div className={'hidden md:flex relative cursor-pointer'}>
                                <div className={`group relative ${poppins.bold.className}`} onMouseLeave={()=>{
                                    setTimeout(()=>{
                ...
                <div style={{position:'fixed'}} className={`lg:hidden fixed left-0 px-8 py-5 transition-all ${gameModalIslOpen ? 'bottom-0 opacity-100 visible' : 'bottom-[-100vh] invisible opacity-0'} items-center justify-center w-[100vw] bg-primary z-10`}>
                    <div
                        ...
                        >
                            <div className={`flex gap-3 items-center p-3 py-5 text-sm ${poppins.bold.className} border-b border-primary-light hover:bg-primary-dark`}>
                                <GiNinjaHeroicStance size={32} />
                                <span>Heroes</span>
                            </div>
                        </Link>
                    ...
                    <button onClick={()=> setGameModalIsOpen(false)} className={'bg-primary-light p-5 w-full rounded-full mt-5'}>
                        Close
                    </button>
                </div>
                {/*   BOTTOM GAME MODAL PANEL  */}
